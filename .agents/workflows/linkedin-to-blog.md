...
1. **Original URL & Text sichern**: Lade den rohen Content der URL ZWINGEND zuerst über das Skript `node scrape_linkedin.cjs "URL"`. 
   - **BILDER UND DATEN (PUPPETEER)**: Das Skript nutzt Puppeteer, führt JavaScript aus und extrahiert den Text, die echten Kommentare UND das Lazy-Loaded-Bild (als sauberes JSON-Objekt in der Konsole). Nutze dies als primäre Datenquelle!
   - **FALLBACK-METHODE**: Nur wenn das Skript fehlschlägt, nutze `read_url_content`. Auch wenn die sofortige Konsolenausgabe abgeschnitten ist, speichert das Tool die **komplette HTML-Struktur (Guest-View)** als Markdown-Datei ab. Nutze `view_file`, um diese zwischengespeicherte Datei vollständig zu lesen und unten die echten Kommentare zu finden. Halluziniere niemals Kommentare! 
   - **DATUMS-TRICK (Snowflake ID)**: LinkedIn nutzt Snowflake IDs. Du kannst das exakte Publikationsdatum mathematisch aus der URN-ID im Link (z.B. `7489304989883039744`) berechnen, indem du die ID um 22 Bit nach rechts verschiebst: `python3 -c "import datetime; print(datetime.datetime.fromtimestamp((7489304989883039744 >> 22) / 1000.0, tz=datetime.timezone.utc).strftime('%d.%m.%Y'))"`. Nutze diesen Trick IMMER, um das exakte Datum des Posts herauszufinden, ohne Jörg fragen zu müssen!
