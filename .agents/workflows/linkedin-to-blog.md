...
1. **Original URL & Text sichern**: Lade den rohen Content der URL via `read_url_content` (oder per Skript/Embed).
   - **STRIKTE EXTRAKTIONS-METHODE (LERNWERT)**: Nutze ZWINGEND `read_url_content` für LinkedIn-URLs. Auch wenn die sofortige Konsolenausgabe abgeschnitten ist, speichert das Tool die **komplette HTML-Struktur (Guest-View)** als Markdown-Datei ab. Nutze `view_file`, um diese zwischengespeicherte Datei vollständig zu lesen. Dort findest du unten im Dokument **die echten Kommentare**. Halluziniere niemals Kommentare! 
