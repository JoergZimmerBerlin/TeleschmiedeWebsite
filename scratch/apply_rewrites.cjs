...
for (const rw of rewrites) {
  ...
  // Replace meta_title
  if (content.match(/^meta_title:/m)) {
    content = content.replace(/^meta_title:\s*(["']?).*?\1$/m, \`meta_title: "\${rw.meta_title}"\`);
  } else {
    // Insert meta_title after title
    content = content.replace(/^title:(.*)$/m, \`title:$1\\nmeta_title: "\${rw.meta_title}"\`);
  }
  
  // Replace description
  content = content.replace(/^description:\s*(["']?).*?\1$/m, \`description: "\${rw.description}"\`);
