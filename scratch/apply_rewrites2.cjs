...
for (const rw of rewrites) {
  const filePath = path.join(basePath, rw.file);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('meta_title:')) {
      newLines.push(\`meta_title: "\${rw.meta_title}"\`);
      hasMetaTitle = true;
    } else if (line.startsWith('description:')) {
      newLines.push(\`description: "\${rw.description}"\`);
    } else {
      newLines.push(line);
      if (line.startsWith('title:') && !hasMetaTitle) {
        // Just in case it didn't exist
        // We will push meta_title if we don't find it later, but wait, 
        // a safer way is to check if meta_title exists first.
      }
    }
  }
  ...
  if (!hasMetaTitle) {
    ...
    if (titleIdx > -1) {
      newLines.splice(titleIdx + 1, 0, \`meta_title: "\${rw.meta_title}"\`);
    }
  }
  
  fs.writeFileSync(filePath, newLines.join('\n'), 'utf-8');
  console.log('Updated ' + rw.file);
}
