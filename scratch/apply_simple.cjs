...
for (const rw of rewrites) {
  let c = fs.readFileSync(rw.file, 'utf8');
  
  if (c.match(/^meta_title:/m)) {
    c = c.replace(/^meta_title:.*?$/m, \`meta_title: "\${rw.meta_title}"\`);
  } else {
    c = c.replace(/^title:(.*)$/m, \`title:$1\\nmeta_title: "\${rw.meta_title}"\`);
  }
  
  c = c.replace(/^description:.*?$/m, \`description: "\${rw.description}"\`);
  
  fs.writeFileSync(rw.file, c);
  console.log('Processed ' + rw.file);
}
