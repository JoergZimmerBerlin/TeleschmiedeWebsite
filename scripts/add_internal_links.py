...
dir_path = '/Users/jorti/Documents/Antigravity/src/content/blog'
...
mappings = {
  ...
  'seo-sprechstunde-so-laeuft-sie-ab.md': [
    { 'title': 'Die SEO-Sprechstunde einfach erklärt: Website & Sichtbarkeit auf Grill', 'slug': 'seo-sprechstunde-erklaert' },
    { 'title': '5 Sterne: Was Kunden über die SEO-Sprechstunde sagen', 'slug': 'seo-sprechstunde-bewertung-ronny' }
  ],
...
for filename, links in mappings.items():
    ...
    if "### Weiterführende Artikel" in content:
    ...
    for link in links:
        link_block += f"* **Lese-Tipp:** [{link['title']}](/blog/{link['slug']}/)\n"
        
    hr_index = content.rfind('\\n---')
    if hr_index != -1 and hr_index > len(content) - 200:
        content = content[:hr_index] + link_block + content[hr_index:]
    else:
