...
# Die 40 handverlesenen, authentischen Jörg-Zitate (Scraped + Liste)
quotes = [
...
def clean_old_quotes(text):
    # Regex to find and remove old quote div blocks.
    # We look for <div ...> ... 💬 ... </div>
    pattern = r'<div class="my-8 bg-lime-accent/10 border-l-4 border-lime-600 p-6 rounded-r-lg">\s*<p class="font-bold text-lime-600 mb-2">💬.*?</p>\s*<p class="italic text-dark mb-0">".*?"</p>\s*</div>\n*'
...
for file_path in files:
    ...
    # Clean old quotes first
    cleaned_content = clean_old_quotes(content)
...
print(f"Purged old quotes and beautifully injected {len(quotes)} unique LinkedIn insights into {injected_count} glossary articles!")
